...
request_num_seq = Sequence("request_num_sec", start = 1)

class Request(Base):
    __tablename__ = "requests"

    id = Column(Integer, primary_key=True, index=True)
    request_num = Column(Integer, request_num_seq, server_default=request_num_seq.next_value(), unique=True, nullable=False)
    status = Column(String, nullable=False, default="Открыт")
    ol_count = Column(Integer, nullable=True)
    customer_id = Column(Integer, ForeignKey("requests.customer_id", ondelete="SET NULL"), nullable=True) # Связь через внешний ключ
    organization_id = Column(Integer, ForeignKey("requests.organization_id", ondelete="SET NULL"), nullable=True) #Связь через внешний ключ
    request_purpose = Column(Text, nullable=False)
    description = Column(Text, nullable=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    edited_at = Column(DateTime,onupdate=func.now(), nullable=True)
    ...
    user_id = Column(Integer, ForeignKey("requests.user_id", ondelete="RESTRICT"), nullable=False) # Связь через внешний ключ
