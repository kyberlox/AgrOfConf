<template>
    <div class="h-[88vh] bg-white p-[32px] rounded-lg w-full">
        <div class="flex justify-end">
            <BaseButton :buttonSettings="{ class: 'button-primary' }" @click="showAddModal = true">
                Добавить
            </BaseButton>
        </div>

        <div class="flex flex-row gap-[16px] mt-[10px]">
            <div class="cursor-pointer bg-white relative" v-for="product in products" :key="product.id">
                <RouterLink
                    class="w-[200px] h-[274] p-[17px] cursor-pointer flex flex-col gap-[10px] border border-[#EAECEF] rounded-[8px] h-full hover:border-orange-500 duration-300 transition-all"
                    :to="{ name: 'productEdit', params: { id: product.id } }">
                    <div
                        class="bg-contain bg-no-repeat bg-center w-full h-[170px]"
...
export default defineComponent({
    ...
    setup() {
        ...
        const changeProduct = (type: "add" | "edit", id: number | null, userInputs: ProductForm) => {
            switch (type) {
                case "add":
        ...
        const addProduct = async (userInputs: ProductForm) => {
        ...
        const editProduct = async (id: number, userInputs: ProductForm) => {
            isLoading.value = true;
            const formInput = new FormData();
            formInput.append("name", userInputs.name);
            formInput.append("description", userInputs.description || "");
