  ...
  <div class="border border-gray-200 p-[20px] rounded-xl">
    <div class="flex flex-row flex-wrap items-center justify-between gap-[12px] mb-[12px]">
      ...
      <div class="flex flex-row gap-[8px] flex-wrap">
        <BaseButton :buttonSettings="{ class: 'button-secondary' }" @clicked="createBlock">
          Создать блок
        </BaseButton>
        <BaseButton :buttonSettings="{ class: 'button-primary' }" @clicked="generateContacts">
    ...
    <p class="text-sm text-gray-500 mb-[12px]">
      Перетащите параметры в нужный блок (или обратно в «Не распределены»). Порядок блоков
      меняется стрелками. Свойства блока (редактируемость, видимость) применяются ко всем
      его параметрам.
    </p>
    ...
    <div v-else class="flex flex-col gap-[12px]">
      ...
      <div class="border border-dashed border-gray-300 rounded-xl p-[12px]" @dragover.prevent
        ...
        <div class="flex flex-row flex-wrap gap-[8px]">
          <span v-for="p in unassigned" :key="p.id" draggable="true" class="param-chip"
            :class="{ 'opacity-40': isDragging(p) }" @dragstart="dragStart(p, null)" @dragend="dragEnd">
            {{ p.name }}
          </span>
          <span v-if="!unassigned.length" class="text-xs text-gray-400">Перетащите сюда параметры, чтобы убрать их из
            блоков</span>
        </div>
      </div>
      ...
      <div v-for="(block, index) in blocks" :key="block.id"
        class="border border-blue-200 bg-blue-50/40 rounded-xl p-[12px]" @dragover.prevent
        ...
        <div class="flex flex-row items-center justify-between gap-[8px] mb-[8px] flex-wrap">
          ...
          <div class="flex flex-row items-center gap-[6px] shrink-0">
            <button type="button" title="Вверх"
              ...
              :disabled="index === 0" @click="moveBlock(block, -1)">
              ↑
            </button>
            <button type="button" title="Вниз"
              class="w-[24px] h-[24px] rounded-md bg-white border border-gray-200 text-gray-600 hover:bg-gray-100"
              :disabled="index === blocks.length - 1" @click="moveBlock(block, 1)">
              ↓
            </button>
        ...
        <div class="flex flex-row flex-wrap gap-[8px] min-h-[40px]">
          <span v-for="p in block.params" :key="p.id" draggable="true" class="param-chip param-chip--in-block"
            :class="{ 'opacity-40': isDragging(p) }" @dragstart="dragStart(p, block.id)" @dragend="dragEnd">
            {{ p.name }}
          </span>
    ...
    <div v-if="editingBlock" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div class="bg-white rounded-lg p-6 w-full max-w-md shadow-xl">
        ...
        <div class="flex justify-end gap-3 mt-5">
          <button class="px-4 py-2 rounded bg-gray-200 hover:bg-gray-300" @click="editingBlock = null">
            Назад
          </button>
          <button class="px-4 py-2 rounded text-white bg-orange-500 hover:bg-orange-600" @click="saveBlock">
            Сохранить
          </button>
        </div>
      </div>
  ...
  setup(props, { emit }) {
    ...
    const createBlock = async () => {
      ...
      try {
        ...
        alert(detail || 'Не удалось создать блок')
      }
    }
    ...
    const saveBlock = async () => {
      ...
      try {
        ...
        const detail = (error as AxiosError<{ detail?: string }>).response?.data?.detail
        alert(detail || 'Не удалось сохранить блок')
      }
    }

    const deleteBlock = async (block: ParamBlock) => {
      if (!window.confirm(`Удалить блок «${block.name}»? Его параметры станут не распределёнными.`))
        return
      try {
