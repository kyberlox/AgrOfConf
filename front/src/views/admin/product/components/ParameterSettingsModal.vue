    ...
    <SlotModal @closeModal="$emit('closeModal')">
        <div class="flex flex-col w-full gap-[18px] min-w-[750px] p-[16px]">
            ...
            <div v-if="parameter?.type == 'FormulaMix'" class="text-xs text-gray-500">
                Это параметр состава смеси: в конфигураторе для него откроется попап-редактор рабочих сред (выбор сред и
                мольных долей, сумма = 100%).
            </div>
            ...
            <template v-if="parameter?.type == 'Formula'">
                <div v-for="field in formulaFields" :key="field.key" class="flex flex-col gap-[8px]">
                    <label class="text-sm text-gray-700">{{ field.label }}</label>
                    <input
                        class="input-param w-full"
                        :list="field.listId"
                        :value="formulaConfig[field.key]"
                        :placeholder="field.placeholder"
                        @input="changeValue(($event.target as HTMLInputElement).value, field.key)" />
                ...
                <ValuesListEditor
                    v-if="['list', 'select-input'].includes(newParameter.required_type)"
                    ...
                    <div v-if="files.length" class="flex flex-col gap-[6px]">
                        <div
                            ...
                            <div class="flex flex-row items-center gap-[8px] min-w-0">
                                <img
                                    v-if="isImage(f.name) && f.file_url"
                                    :src="fileUrl(f.file_url)"
                                    class="h-10 w-10 object-contain rounded border border-gray-200"
                                    alt="" />
                                <span class="truncate">{{ f.name }}</span>
                            </div>
                            <button class="text-red-500 hover:text-red-700" @click="removeFile(f)">Удалить</button>
                        </div>
                    </div>
                    <input type="file" multiple class="input-param" @change="uploadFiles" />
                </div>
            </template>

            <div class="flex flex-row justify-end gap-[15px]">
                <div v-for="(item, index) in ['Назад', 'Принять']" :key="'bread' + index">
                    <BaseButton
                        class="min-w-[200px]"
                        :buttonSettings="{
                            class: item == 'Назад' ? 'button-secondary' : 'button-primary',
