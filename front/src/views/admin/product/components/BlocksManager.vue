<template>
    <div class="border border-gray-200 p-[20px] rounded-xl">
        ...
        <p class="text-sm text-gray-500 mb-[12px]">
            Перетащите параметры в нужный блок (или обратно в «Не распределены»). Порядок блоков меняется стрелками.
            Свойства блока (редактируемость, видимость) применяются ко всем его параметрам.
        </p>

        <div v-if="loading" class="engine-params__loader">
            <Loader />
        </div>
        <div
            v-else-if="!blocks.length && !unassigned.length"
        ...
        <div v-else class="flex flex-col gap-[12px]">
            ...
            <div
                class="border border-dashed border-gray-300 rounded-xl p-[12px]"
                @dragover.prevent
                ...
                <div class="flex flex-row flex-wrap gap-[8px]">
                    ...
                    <span v-if="!unassigned.length" class="text-xs text-gray-400"
                        >Перетащите сюда параметры, чтобы убрать их из блоков</span
                    >
                </div>
            </div>
            ...
            <div
                ...
                <div class="flex flex-row items-center justify-between gap-[8px] mb-[8px] flex-wrap">
                    <div class="flex flex-row items-center gap-[8px] min-w-0 flex-wrap">
                        ...
                        <span
                            v-if="block.properties?.editable === false"
                            class="text-[11px] px-[6px] py-[1px] rounded-full bg-orange-100 text-orange-700"
                            >только просмотр</span
                        >
                        <span
                            v-if="block.properties?.visibility === false"
                            class="text-[11px] px-[6px] py-[1px] rounded-full bg-gray-200 text-gray-600"
                            >скрыт</span
                        >
                    </div>
