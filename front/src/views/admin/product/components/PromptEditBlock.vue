<template>
<div class="flex flex-col gap-2 mt-4 border border-gray-200 p-[20px] rounded-xl bg-gray-50"
    ...
    <div v-if="systemPrompt.rules_table?.length"
        ...
        <div class="grid grid-cols-4 gap-[20px]  p-[15px]">
            ...
            <div class="py-[20px]">
                <BaseButton :button-settings="{ class: 'button-secondary' }"
                            @clicked="showParamsModal = true">
                    Добавить
                </BaseButton>
            </div>
        </div>
    </div>
    <BaseButton class="mt-[12px] flex flex-row items-center max-w-fit"
        ...
        <Loader class="max-w-fit"
                v-else />
    </BaseButton>

    <SlotModal v-if="showParamsModal"
               @closeModal="showParamsModal = false">
