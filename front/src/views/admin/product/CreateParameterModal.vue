    ...
    <div class="bg-white rounded-lg p-6 w-full max-w-md shadow-xl">
      ...
      <div class="flex flex-col gap-3">
        ...
        <div v-if="form.type == 'FormulaMix'" class="text-xs text-gray-500">
          Параметр состава смеси. В конфигураторе для него откроется попап-редактор: выбор
          рабочих сред и их мольных долей (двух сред и более, сумма долей = 100%).
        </div>

        <label
          v-if="form.type != 'Drawing' && form.type != 'FormulaMix'"
        ...
        <template v-if="form.type == 'Table'">
          <label class="flex flex-col gap-1 text-sm">
            <span class="text-gray-700"
              >Таблица (таблица, в которую добавить колонку)</span
            >
            <input
              class="input-param w-full"
