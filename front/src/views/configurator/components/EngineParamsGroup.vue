<template>
  <div class="grid grid-cols-1">
    ...
    >
      ...
      <div
        v-if="
          (param as IFormattedData).editable === false &&
          (param as IFormattedData).required_type !== 'drawing'
        "
        class="flex flex-col w-full gap-[4px] py-[6px] px-[4px]"
      >
        <span class="text-[13px] text-[#343B4C] font-[600]">{{ param.name }}</span>
        <span
          class="text-[16px] font-bold text-gray-900"
      ...
      <SelectInput
        v-else-if="
          (param as IFormattedData).required_type == 'select-input' &&
          (param as IFormattedData).type !== 'FormulaMix' &&
          (param as IFormattedData).name != 'Состав смеси'
        "
      ...
      <MixtureEditor
        ...
        :model-value="
          userParams &&
          userParams[param.name as keyof typeof userParams] &&
          Array.isArray(userParams[param.name as keyof typeof userParams])
            ? (userParams[param.name as keyof typeof userParams] as Array<{
                [key: string]: number
              }>)
            : []
        "
        :disabled="paramsLoading"
        @valueChanged="
          (value: Array<{ [key: string]: number }>) => $emit('valueChanged', value, param.name)
        "
      />

      <!-- Чекбокс (например, включение расчёта смеси) -->
      <CheckboxInput
        v-else-if="(param as IFormattedData).required_type == 'checkbox'"
        :param="param as IFormattedData"
      ...
      <BaseInput
        v-else-if="(param as IFormattedData).required_type == 'user_input'"
        :class="{
          'input-param__wrapper--no-response':
            userParams && !userParams[param.name as keyof typeof userParams],
        }"
      ...
      >
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          class="text-[12px] text-blue-600 underline"
          >Открыть чертеж в новой вкладке</a
        >
        <span v-else class="text-[12px] text-gray-400"
          >Чертеж будет доступен после выбора параметров</span
        >
      </div>
      ...
      <BaseSelect
        v-else
        :class="{
          'select-params__wrapper--no-response':
            userParams && !userParams[param.name as keyof typeof userParams],
        }"
  ...
  setup(props) {
    ...
    const checkParams = (param: IFormattedData) => {
      const norm = (arr?: string[]) =>
        Array.isArray(arr) ? arr.map((e) => replaceSpotOrComma(e, 'spot')) : []
    ...
    const setPropsValue = (param: IFormattedData): string => {
      ...
      const value = Array.isArray(raw) || typeof raw === 'boolean' ? String(raw) : raw
      if (value) {
        return replaceSpotOrComma(value, 'spot') || ''
      } else if (typeof param?.response_value === 'string')
    ...
    const initSelectSettings = (param: IFormattedData) => {
      return {
        ...
        placeholder:
          !param.filtered_values?.length && 'filtered_values' in param ? '' : 'Выберите значение',
        needReq: true,
        labelIcon: createLabelIconsComponent(param, () => console.log('testComp')),
        error: 'error' in param ? param.error : '',
        errorIcon: AlertCircle,
        disabled:
          (((!param.filtered_values?.length && 'filtered_values' in param) ||
            (param as IFormattedData).filtered_values?.includes('нет')) &&
            props.type == 'auto') ||
          props.paramsLoading ||
