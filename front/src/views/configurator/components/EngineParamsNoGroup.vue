<template>
  <div
    class="grid 3xl:grid-cols-4 xl:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-x-[6px] gap-y-[16px]"
  >
    <div v-for="(param, index) in items" :key="param.id" class="px-[10px] hover:none flex flex-row">
  ...
  setup(props) {
    ...
    const checkParams = (param: IFormattedData) => {
      switch (freeConfigMode.value) {
        ...
        case false:
          return Array.from(
            !('filtered_values' in param) || !param?.filtered_values || param.response_value
