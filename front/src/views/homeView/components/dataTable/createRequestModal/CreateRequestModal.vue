    ...
    <SlotModal>
        <div class="p-[40px] max-h-[90vh]">
            ...
            <div class="grid grid-cols-2 gap-[16px] mt-[16px]">
                <div
                    v-for="(customerType, index) in costumersRef"
                    ...
                    <div v-if="!customerType.hidden" class="font-semibold">
                        {{ customerType.title }}
                    </div>
                    <div class="flex flex-col gap-[8px] mt-[24px]">
                        <BaseButton v-if="customerType.hidden" @clicked="customerType.hidden = false">
                        ...
                        <div
                            ...
                            <div v-if="question.title == 'Срок'" class="flex flex-row gap-[8px]">
                                Срок ТКП
                                <input type="date" />
                                Срок поставки
                                <input type="date" />
            ...
            <div class="max-w-fit ml-auto mt-[16px] pb-[16px]">
                <BaseButton :buttonSettings="{ class: 'button-primary' }"> Создать </BaseButton>
            </div>
        </div>
    </SlotModal>
</template>
...
import { requestFields, customers, customerFields } from "@/assets/static/createRequestData";
...
export default defineComponent({
    ...
    setup() {
        ...
        const initSelectProps = (
            question: string,
            customerType: { title: string; options: Array<string> },
            index: string,
        ) => {
