<template>
    <div class="flex flex-row items-center justify-between pb-[15px]">
        <div class="mt-[24px] pl-[24px] flex flex-row gap-[12px] items-center text-[12px]" v-if="canPageSplit">
        ...
        <div class="pr-[24px] flex flex-row gap-[8px] items-center text-[14px] ml-auto">
            <div
                v-if="Number(activePage) !== 1"
                class="w-[24px] h-[24px] cursor-pointer flex items-center justify-center content-center"
...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const initPagination = () => {
            ...
            if (!pagination.includes(props.total) && pagination.length > 2) {
                pagination.push("...", props.total);
            } else if (pagination.length > 3) {
