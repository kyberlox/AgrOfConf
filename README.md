...
- **Перенос продуктов** — экспорт полной конфигурации продукта (таблицы, файлы,
  параметры, в том числе формульные конфигурации) в ZIP и импорт обратно.
- **Расчёт диаметра седла предохранительного клапана (ПК)** — по ГОСТ 12.2.085
  (прил. Д): отдельные ветки для газа, жидкости и **двухфазного потока**
  (метод Ω, ISO 4126‑10), предварительный подбор строки таблицы клапана
  (`valve_selection`) и заполнение её колонок в подборе.
- **Смеси рабочих сред** — тип параметра `FormulaMix` (чекбокс «Смесь»,
  select «Тип смеси», параметр «Состав смеси»): характеристики смеси (включая
  двухфазную ветку с методом Ω), валидация состава (сумма долей = 100%, ≥ 2 сред).
...
| `router/product_porting.py` | Экспорт/импорт продукта в ZIP (`/api/products/{id}/export`, `/api/products/import`): параметры (включая `formula_config`, блоки, таблицы, файлы) |
...
| Файл | Назначение |
|------|-----------|
| `algorithms.py` | Библиотека функций расчёта: общие (`count_A`, `area_of_circle`), характеристики сред/смесей (`mixture_*`), расчёт седла ПК (`valve_*`, `seat_*`), подбор таблицы клапана (`valve_selection`) |
| `validators.py` | Библиотека функций валидации (`validate_nonzero`, `validate_T_PK`, `validate_pressure_setting`, `validate_backpressure`, `validate_mixture_composition`) |
...
| `integration.py` | Интеграция в `module_search` (новые формулы + fallback), смеси, чертежи, подбор клапана |
...
- `/api/products/{id}/export`, `/api/products/import` — перенос продукта (ZIP:
  параметры, блоки, `formula_config`, таблицы, файлы)
...
Вместо жёстко зашитых русских имён параметров функции используют **ключевые
слова**: имя ищется среди фактических параметров продукта, поэтому переименование
параметра или добавление суффикса единиц измерения не ломает расчёт.

- `_actual_param_name(ctx, config, key, *keywords, exclude=())` — фактическое
  имя по ключевым словам; имя можно переопределить в `formula_config`
  (например `"pn_param": "Давление настройки"`).
- `_required_value_by_keyword(ctx, keywords, exclude)` — значение первого
  совпавшего параметра, с корректным ожиданием формульных результатов
  (через `MissingParamError`).
- Вся нормализация идёт через `_norm_lower` (**нижний регистр + замена
  «ё»→«е»**), поэтому «Теплоемкость» и «Теплоёмкость» считаются одним словом.
- В валидаторах (синхронных, без БД) имя находится через `_find_actual_key(ctx,
  *keywords, fallback)`.

### Смеси (тип параметра `FormulaMix`)

Если у продукта заведён параметр состава смеси (`type='FormulaMix'`, по
умолчанию «Состав смеси»), подбор работает в двух режимах:

- **Обычная среда** — чекбокс «Смесь» выключен: считаются характеристики
  выбранной среды;
- **Смесь** — чекбокс «Смесь» включён и выбран «Тип смеси»: характеристики
  вычисляются из состава «среда: мольная доля» (минимум 2 среды, сумма = 100%),
  агрегатное состояние смеси может быть «Газ», «Жидкость» или
  «Двухфазный поток».

Функции `mixture_*` (`mixture_state`, `mixture_density`, `mixture_molar_mass`,
`mixture_viscosity`, `mixture_adiabatic_index`, `mixture_isobaric_capacity`,
`mixture_isochoric_capacity`, `mixture_latent_heat`, `mixture_factor`,
`mixture_material`, `mixture_characteristics`) читают характеристики каждой среды
из таблицы сред и усредняют по составу (объёмные доли → массовые, лог-микс
вязкости и т.п.). Результат кэшируется в `ctx.computed["_mixture_full"]`.

Настройка состава возможна двумя способами в `formula_config`:
- `mixture_param` / `type_param` / `composition_param` — имена чекбокса «Смесь»,
  select «Тип смеси» и параметра-состава (по умолчанию «Смесь» / «Тип смеси» /
  «Состав смеси»);
- `slots` — список имён слотов «среда/доля» (чередуются пары) для продуктов со
  старой настройкой.

### Расчёт диаметра седла предохранительного клапана

Полный расчёт `_seat_calc_full` реализует приложение Д ГОСТ 12.2.085 с выбором
ветки по агрегатному состоянию среды:

- **Газ** — плотность по ИУГ, критические коэффициенты ПВД (докрит./сверхкрит.
  истечение), `Kw` по противодавлению, `Kp_kr`/`Kb`;
- **Жидкость** — плотность жидкой фазы, `Kw` (0.875+1.8333·ratio−6.6667·ratio²
  при `ratio ≤ 0.25`, иначе 1.149−0.988·ratio), `Kp = √(2·(1−B))`;
- **Двухфазный поток** — **метод Ω (ISO 4126‑10)**: параметр
  `_omega_parameter`, критическое отношение `ηc = 2Ω/(2Ω+1)`, массовая скорость
  `_two_phase_mass_flux` (критический/докритический режим).

Далее — итерационный расчёт диаметра седла с учётом числа Рейнольдса и
коэффициента расхода `Kv`, с округлением вверх до 0.1 мм. Входные параметры
ищутся по ключевым словам (см. выше), поэтому переименования
«Устройство принудительного открытия», «Давление настройки», «Максимальный
аварийный расход», «Противодавление статическое», «Температура рабочей среды»
и т.п. учитываются автоматически.

Итог доступен через функции-витрины: `valve_seat_diameter`, `valve_seat_area`,
`valve_effective_area`, `valve_mass_velocity`, `valve_start_pressure`,
`valve_full_pressure`, `valve_inlet_pressure`, `valve_outlet_pressure`,
`valve_kw`, `valve_vapor_quality`, `valve_omega`, `valve_critical_ratio`.
Числа давления пересчитываются из МПа в кгс/см² (×10.197162).

### Предварительный подбор таблицы клапана

`valve_selection` — формула-драйвер: по выбранным «Типу клапана»,
«Предварительному диаметру седла» и «Предварительному номинальному давлению»
подбирает строку таблицы клапана («Тип ПК» совпадает, диаметр седла —
минимально подходящий, PN входное — совпадает) и возвращает «Номинальный
диаметр седла клапана, мм». Результат кэшируется в `ctx.computed["_valve_selection"]`,
интеграция записывает его в колонки таблицы клапана (Тип ПК, PN входное/выходное,
DN входной/выходной, диапазон давления настройки, № и материал пружины),
помечает их нередактируемыми и скрывает. `seat_circle_area` /
`seat_effective_area` считают площадь седла по этому диаметру.
